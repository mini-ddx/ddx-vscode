...
export function getStylusMode(documentRegions: LanguageModelCache<VueDocumentRegions>): LanguageMode {
  ...
  return {
    getId: () => 'stylus',
    configure(c) {
      baseIndentShifted = _.get(c, 'ddx.format.styleInitialIndent', false);
      config = c;
    },
    onDocumentRemoved() {},
    dispose() {},
    ...
    format(document, range, formatParams) {
      if (config.ddx.format.defaultFormatter.stylus === 'none') {
        return [];
      }
      ...
      const vlsFormatConfig = config.ddx.format as VLSFormatConfig;
      ...
      // Add one more indentation when `ddx.format.styleInitialIndent` is set to `true`
